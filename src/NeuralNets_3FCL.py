...
class CNN_4_Layers(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn4( F.relu(F.max_pool2d(self.conv4(x), 2)) )
        ...
        self.emb2 = self.fc2( self.emb )

        x = F.dropout(self.emb2, 0.5, training=self.training)
        x = F.relu(x);
        

        #x = F.dropout(self.emb, 0.5, training=self.training)
        #x = F.relu(x)
